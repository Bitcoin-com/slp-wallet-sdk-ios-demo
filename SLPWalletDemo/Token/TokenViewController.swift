...
class TokenViewController: UIViewController {
    ...
    //
    // VIEW -> Presenter?
    // -> onStart() -> presenter?.onStart() -> Load... -> self.onResume()
    // -> onResume() -> presenter?.onResume() -> Get last informations // Heavy! =>
    //
    // viewDelegate?.onGetBalance() // Balance changed?
    // viewDelegate?.onGetBalance() // Balance changed?
    // viewDelegate?.onGetBalance() // Balance changed?
    // viewDelegate?.onGetBalance() // Balance changed?
    // viewDelegate?.onGetBalance() // Balance changed?
    // viewDelegate?.onGetBalance() // Balance changed?
    //
    // -> onPause() -> presenter?.onPause() -> Don't listen anymore?
    //
    // Presenter -> subscribe -> viewDelegate?.onGetBalance() -> My view is not there!
    // // // //
    // // // //
